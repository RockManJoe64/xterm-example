# xterm-example

<p float="left">
    <img alt="xterm logo" src="https://raw.githubusercontent.com/xtermjs/xtermjs.org/refs/heads/master/images/favicon.png" height="32">
    <img  alt="pnpm logo" src="https://pnpm.io/img/pnpm-no-name-with-frame.svg" height="32">
    <img  alt="parcel logo" src="https://avatars.githubusercontent.com/u/32607881?s=200&v=4" height="32">
    <img alt="biome logo" src="https://avatars.githubusercontent.com/u/140182603?s=48&v=4" height="32">
</p>

An example web app using [xtermjs](https://xtermjs.org/).

- This app uses [pnpm](https://pnpm.io/) for dependency management.
- It also uses [Parcel](https://parceljs.org/) to test and build.
- The app uses [biome](https://biomejs.dev/) for linting and formatting.
- In the future may switch to using [Volta](https://volta.sh/)

To start the app, run the command

```bash
pnpm run start
```
